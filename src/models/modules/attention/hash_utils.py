...
class XBOXPLUS(AsymmetricTransform):
    ...
    def set_norms(self, queries, keys):
        self.q_norms = queries.norm(p=2, dim=-1, keepdim=True)
        self.MQ = torch.amax(self.q_norms, dim=-2, keepdim=True)
        self.k_norms = keys.norm(p=2, dim=-1, keepdim=True)
        self.MK = torch.amax(self.k_norms, dim=-2, keepdim=True)
...
class XBOXMax(AsymmetricTransform):
    ...
    def set_norms(self, queries, keys):
        ...
        self.MQ_sq = torch.amax(self.q_norm_sq, dim=-2, keepdim=True)
        self.MK_sq = torch.amax(self.k_norm_sq, dim=-2, keepdim=True)

    def K(self, x):
        device = x.device
        ext = torch.sqrt((torch.maximum(self.MQ_sq.to(device), self.MK_sq.to(device)) - self.k_norm_sq.to(device)))
        return torch.cat([x, ext, torch.zeros_like(ext)], dim=-1)

    def Q(self, x):
        device = x.device
        ext = torch.sqrt((torch.maximum(self.MQ_sq.to(device), self.MK_sq.to(device)) - self.q_norm_sq.to(device)))
...
def lsh_clustering(queries, keys, n_hashes, r=1, key_padding_mask=None):
    ...
    queries_hashed = e2lsh(queries).reshape((n_hashes,) + queries.shape[:-1])
    keys_hashed = e2lsh(keys).reshape((n_hashes,) + keys.shape[:-1])
