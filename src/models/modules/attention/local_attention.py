...
class LocalAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None, key_padding_mask=None, need_weights=False):
        ...
        if key_padding_mask is None:
            key_padding_mask = LengthMask(key.new_full((key.shape[0],), key.shape[1],
                                                       dtype=torch.long))
        ...
        if not key_padding_mask.all_ones:
            ...
            valid_idx_mask = ((local_idx >= 0)
                              & (local_idx < rearrange(key_padding_mask.lengths, 'b -> b 1 1 1')))
            attn_local.masked_fill_(~valid_idx_mask, 0.0)
        ...
        if need_weights:
            ...
            local_mask = ((idx >= -(self.local_context // 2))
                          ...
                          & (k < rearrange(key_padding_mask.lengths, 'b -> b 1 1 1')))
